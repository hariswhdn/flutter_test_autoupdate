...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<Map<String, dynamic>> init2() async {
    try {
      final http.Response res = await http
          .get(Uri.parse('https://apimocha.com/oiugsad/test'), headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      });
      if (res.statusCode >= 200 && res.statusCode <= 299) {
        final Map<String, dynamic> data = await json.decode(res.body) ?? {};
        return data;
      } else {
        throw Error();
      }
  ...
  void init() async {
    try {
      final Map<String, dynamic> data = await init2();
      if ((data['data'] ?? []).isNotEmpty) {
        final UpdateResult updateResult =
            UpdateResult.fromJson(data['data'][0]);
              ...
              builder: (BuildContext ctx) {
                return AlertDialog(
                  ...
                  actions: [
                    ...
                    TextButton(
                      child: const Text('Tidak, keluar dari aplikasi'),
                      onPressed: () {
                        SystemNavigator.pop();
                      },
                    ),
